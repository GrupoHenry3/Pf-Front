...
interface PetContextType {
    ...
    refreshShelterPets: () => Promise<void>;
}
...
export const PetProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const refreshShelterPets = async () => {
        if (user && user.userType === "Shelter" && user.shelter?.id) {
            try {
                setIsPetLoading(true);
                const pets = await petsService.findAllByShelter(user.shelter.id);
                ...
                console.error("Error al refrescar mascotas del shelter:", error);
            } finally {
                setIsPetLoading(false);
            }
        }
    ...
    return(
        <PetContext.Provider value={{ shelterPets, setShelterPets, setAllPets, allPets, setIsPetLoading, latestPets, setLatestPets, petsToAdopt, setPetsToAdopt, isPetLoading, refreshShelterPets }}>
            {children}
        </PetContext.Provider>
    )
}
