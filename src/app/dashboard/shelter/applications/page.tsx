...
const ShelterApplicationsView = () => {
  const {shelterAdoptions} = useAdoption();
  const {user} = useUser();
        ...
        <CardContent>
          <div className="rounded-md border">
            <Table>
              ...
              <TableBody>
                {shelterAdoptions?.map((application) => (
                  <TableRow key={application.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-3">
                        <ImageWithFallback
                          src={user?.shelter?.avatarURL}
                         alt={user?.shelter?.name}
                          className="w-8 h-8 rounded-lg object-cover"
                        />
                        <div>
                          <div className="font-medium">{user?.shelter?.name}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => router.push(`/dashboard/shelter/adoption/${application.id}`)}
                      >
          ...
          {(!shelterAdoptions || shelterAdoptions.length === 0) && (
...
export default ShelterApplicationsView;