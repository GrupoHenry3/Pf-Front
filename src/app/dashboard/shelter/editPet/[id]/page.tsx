...
export default function EditPetPage({ params }: EditPetPageProps) {
    ...
    const loadPet = async () => {
      try {
        ...
        setFormData({
          name: petData.name || "",
          age: petData.age || 0,
          size: (petData.size?.toLowerCase() as "small" | "medium" | "large") || "small",
          gender: (petData.gender?.toLowerCase() as "male" | "female") || "male",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pets/${id}`, {
        method: "PATCH",
