        ...
        <div className="flex-1">
          <div className="max-w-2xl mx-auto px-4 py-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-5 flex items-center gap-4 mb-8" style={{ minHeight: '90px' }}>
              <img src={mockPet.images[0]} alt={mockPet.name} className="w-16 h-16 object-cover rounded-lg" />
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-semibold text-gray-900">{mockPet.name}</h2>
                </div>
                <p className="text-gray-600 text-sm mt-1">{mockPet.breed} • {mockPet.age} años • {mockPet.location}</p>
                <span className="inline-block mt-2 px-3 py-1 rounded-full bg-green-100 text-green-700 text-xs font-medium">{mockPet.shelterName}</span>
              </div>
            </div>
