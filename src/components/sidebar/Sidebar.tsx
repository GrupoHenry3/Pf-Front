...
import { Menu, Home, Search, Heart, MessageCircle, LogOut, User, FileText, Building2 } from "lucide-react";
...
export function Sidebar({ user }: SidebarProps) {
  ...
  const getNavigationItems = () => {
    const commonItems = [{ id: "donations", label: "Hacer Donación", icon: Heart, badge: null }];
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
          ...
          <div className="flex items-center space-x-3">
            <Avatar className="w-10 h-10">
              {user.avatarURL ? (
                ...
                <AvatarFallback className="bg-green-500 text-white">{user.fullName?.slice(0, 2).toUpperCase()}</AvatarFallback>
              )}
            </Avatar>
        ...
        <nav className="p-4 space-y-2">
          {navigationItems.map((item) => (
            ...
            >
              ...
              {item.label}
              {item.badge && item.badge > 0 && <Badge className="ml-auto bg-orange-500 text-white">{item.badge}</Badge>}
            </Button>
          ))}
        </nav>
      </div>
      <div className="p-4 border-t border-gray-200">
        {user ? (
          <Button variant="ghost" className="w-full justify-start text-red-600 hover:bg-red-500" onClick={handleLogout}>
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side="left" className="p-0 w-64 sm:w-80">
            <SheetHeader className="sr-only">
              ...
              <SheetDescription>Navegación principal de PetAdopt para acceso a todas las funciones</SheetDescription>
            </SheetHeader>
            <SidebarContent />
          </SheetContent>
        </Sheet>
