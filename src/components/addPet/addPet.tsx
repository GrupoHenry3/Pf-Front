...
export function AddPet({ onBack, onSuccess }: AddPetProps) {
  ...
  const { refreshShelterPets } = usePet();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validaciones
      if (!user?.shelter?.id) {
        throw new Error("No se encontró el ID del refugio");
      }
      if (!selectedBreedId || !selectedSpeciesId) {
        throw new Error("Debes seleccionar una especie y raza");
      }
      if (!formData.name || !formData.age || !formData.gender || !formData.size) {
      ...
      console.log("información que va al back de la mascota", petData);
      await petsService.create(petData);
      
      // Refrescar la lista de mascotas del refugio
      await refreshShelterPets();
