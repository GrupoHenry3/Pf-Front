  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        {/* Tabla de donaciones */}
        {/*<div className="text-2xl">Historial de Donaciones</div>*/}
        <Card>
          <CardContent className="pt-6">
                ...
                <TableBody>
                  {donations.reverse().map((donation) => (
                    <TableRow key={donation.id}>
                      <TableCell>
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">
                            <ImageWithFallback src={donation.user.avatarURL || "/placeholder-user.jpg"} alt={donation.user.fullName} className="w-8 h-8 rounded-full object-cover" />
                          </div>
                          <div>
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-3">
                          <ImageWithFallback src={donation.shelter.avatarURL || "/placeholder-pet.jpg"} alt={donation.shelter.name} className="w-8 h-8 rounded-lg object-cover" />
                          <div>
                      ...
                      <TableCell>
                        <div className="text-sm text-gray-600">{new Date(donation.createdAt).toLocaleDateString()}</div>
                      </TableCell>
                      <TableCell>
            ...
            {donations.length === 0 && <div className="text-center py-8 text-gray-500">No hay donaciones registradas</div>}
          </CardContent>
        </Card>
      </div>
    </div>
